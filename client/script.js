...
function colorChange(){
    sayHelloButton.setAttribute('style', 'background-color: black; color: white')
}
...
function colorRevert(){
    sayHelloButton.setAttribute('style', 'background-color: #EFEFEF; color: black;')
}
sayHelloButton.addEventListener('mouseout', colorRevert)
...
const repeatMyParam = () => {
    ...
    .then(res => document.querySelector('#repeat-text').textContent = res.data)
}
...
const queryServer = () => {
    const query = 'name=Steve&color=pink'
    axios.get(`http://localhost:3000/query-test/?${query}`)
        .then(res => console.log(res.data))
}
document.querySelector('#query-button').addEventListener('click', queryServer)
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        Response that said we sent an empty query.

    2: Send more than 1 query on the URL -- what happened? 
        Response that said we sent more than one query.
*/      
...
const createFood = (e) => {
    ...
    axios.post('http://localhost:3000/food', body)
        .then(res => {
            ...
            h1.textContent = `Added ${res.data[res.data.length -1]} to foods!`
            body.appendChild(h1)
        })
